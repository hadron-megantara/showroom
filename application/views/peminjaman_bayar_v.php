	...
	<div class="mainpanel">
		...
		<div class="pageheader">
			...
			<div class="breadcrumb-wrapper">
				<ol class="breadcrumb">
					...
					<li class="active">Bayar Peminjaman</li>
				</ol>
			</div>
		</div>
		<div class="contentpanel">
			<div class="panel panel-default">
				...
				<form id="basicForm" action="<?PHP echo base_url() ?>peminjaman/bayar_data" method="post" name="formsentral">
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<select readonly class="form-control mb15" name="karyawan" required >
									<?PHP
						...
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input readonly type="text" autocomplete="off" name="jumlah" class="form-control" required onkeyup="formatAngka(this, ',');" value="<?PHP $result=mysql_query("SELECT jumlah as data from detail_peminjaman WHERE idpeminjaman='".$iddata."'");$data=mysql_fetch_assoc($result);echo number_format($data['data']);?>" />
							</div>
						</div>
					</div>
					<div class="row">
						...
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input readonly type="text" autocomplete="off" name="catatan" class="form-control" required value="<?PHP $result=mysql_query("SELECT catatan as data from detail_peminjaman WHERE idpeminjaman='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
					</div>
				</div>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
				<h4 class="panel-title">Mohon input pembayaran peminjaman dengan benar</h4>
				</div>
				<form id="basicForm" action="<?PHP echo base_url() ?>peminjaman/bayar_data" method="post" name="formsentral">
				<input type="hidden" name="iddata" value="<?PHP echo $iddata; ?>"/>
				<input type="hidden" name="sebelum" value="<?PHP $result=mysql_query("SELECT dibayar as data from detail_peminjaman WHERE idpeminjaman='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>"/>
				<input type="hidden" name="entry" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("Y-m-d")?>"/>
				<input type="hidden" name="iduser" value="<?PHP echo $this->session->userdata("sm_username")?>"/>
				<input type="hidden" name="tanggaluser" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("d F Y")?>"/>
				<input type="hidden" name="jamuser" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("H:i:s")?>"/>
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input readonly type="text" autocomplete="off" name="sisa" class="form-control" required onkeyup="formatAngka(this, ',');" value="<?PHP $result=mysql_query("SELECT sum(jumlah-dibayar) as data from detail_peminjaman WHERE idpeminjaman='".$iddata."'");$data=mysql_fetch_assoc($result);echo number_format($data['data']);?>" />
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
								<label class="control-label">Dibayar <font color="red">*</font></label>
								<input type="text" autocomplete="off" name="dibayar" class="form-control" required onkeyup="formatAngka(this, ',');" value="" />
							</div>
						</div>
					</div>
				</div>
