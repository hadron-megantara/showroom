	...
	<div class="mainpanel">
		...
		<div class="contentpanel">
			<div class="panel panel-default">
				...
				<h4 class="panel-title">Mohon edit data supplier dengan benar</h4>
				</div>
				<form id="basicForm" action="<?PHP echo base_url() ?>supplier/edit_data" method="post" name="formsentral">
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input type="text" autocomplete="off" name="nama" class="form-control" required value="<?PHP $result=mysql_query("SELECT nama as data from supplier WHERE idsupplier='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
								<label class="control-label">Alamat</label>
								<input type="text" autocomplete="off" name="alamat" class="form-control" value="<?PHP $result=mysql_query("SELECT alamat as data from supplier WHERE idsupplier='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input type="text" autocomplete="off" name="telepon" class="form-control" maxlength="20" onkeypress="return isNumberKey(event)" value="<?PHP $result=mysql_query("SELECT telp as data from supplier WHERE idsupplier='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
					</div>
				</div>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
				<h4 class="panel-title">Anda yakin ingin menghapus supplier</h4>
				</div>
				<form id="basicForm" action="<?PHP echo base_url() ?>supplier/delete_data" method="post" name="formsentral">
				<input type="hidden" name="iddata" value="<?PHP echo $iddata?>"/>
				<input type="hidden" name="entry" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("Y-m-d")?>"/>
				<input type="hidden" name="iduser" value="<?PHP echo $this->session->userdata("sm_username")?>"/>
				<input type="hidden" name="tanggaluser" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("d F Y")?>"/>
