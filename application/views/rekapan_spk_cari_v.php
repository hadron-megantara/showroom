		...
		<div class="pageheader">
			...
			<div class="breadcrumb-wrapper">
				<ol class="breadcrumb">
					...
					<li class="active">Detail Detail SPK</li>
				</ol>
			</div>
		</div>
						...
						<table class="table table-striped mb30" id="table-responsive">
							...
							<tbody>
								<?PHP
									...
									AND penjualan._cabang=".$bind1['data']." ORDER BY idpenjualan DESC");
									if($query->num_rows())
									{
						...
						<table class="table table-striped mb30" id="table-responsive">
							...
							<tbody>
								<?PHP
									$query = $this->db->query("SELECT idpenjualan,pembeli,tipebayar,hsdiscount,tanggalspk,_cabang.data as cabang,penjualan._deleted as deleted
									FROM penjualan,_cabang
									WHERE penjualan._cabang=_cabang.code AND tanggalspk BETWEEN '".$this->input->post("tglawal")."' AND '".$this->input->post("tglakhir")."'ORDER BY idpenjualan DESC");
									if($query->num_rows())
									{
										foreach($query->result() as $row)
										{	
