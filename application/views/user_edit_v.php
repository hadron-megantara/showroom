	...
	<div class="mainpanel">
		...
		<div class="contentpanel">
			<div class="panel panel-default">
				...
				<h4 class="panel-title">Mohon edit data user dengan benar</h4>
				</div>
				...
				<div class="panel-body">
					<div class="row">
						...
						<div class="col-sm-6">
							<div class="form-group">
								<label class="control-label">Jabatan <font color="red">*</font></label>
								<select class="form-control mb15" name="jabatan" required >
									<?PHP
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input type="text" autocomplete="off" name="nama" class="form-control" required value="<?PHP $result=mysql_query("SELECT nama as data from user WHERE username='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
								<label class="control-label">Telepon </label>
								<input type="text" autocomplete="off" name="telepon" class="form-control" maxlength="20" onkeypress="return isNumberKey(event)" value="<?PHP $result=mysql_query("SELECT telepon as data from user WHERE username='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<input type="email" autocomplete="off" name="email" class="form-control" value="<?PHP $result=mysql_query("SELECT email as data from user WHERE username='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
						<div class="col-sm-6">
							<div class="form-group">
								<label class="control-label">Alamat</label>
								<input type="text" autocomplete="off" name="alamat" class="form-control" value="<?PHP $result=mysql_query("SELECT alamat as data from user WHERE username='".$iddata."'");$data=mysql_fetch_assoc($result);echo $data['data'];?>" />
							</div>
						</div>
					</div>
				</div>
			...
			<div class="panel panel-default">
				<div class="panel-heading">
				<h4 class="panel-title">Anda yakin menghapus user</h4>
				</div>
				<form id="basicForm" action="<?PHP echo base_url() ?>user/delete_data" method="post" name="formsentral">
				<input type="hidden" name="entry" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("Y-m-d")?>"/>
				<input type="hidden" name="iduser" value="<?PHP echo $this->session->userdata("sm_username")?>"/>
				<input type="hidden" name="tanggaluser" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("d F Y")?>"/>
				<input type="hidden" name="jamuser" value="<?PHP date_default_timezone_set ("Asia/Jakarta"); echo date("H:i:s")?>"/>
