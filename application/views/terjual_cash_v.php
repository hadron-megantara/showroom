		...
		<div class="pageheader">
			...
			<div class="breadcrumb-wrapper">
				<ol class="breadcrumb">
					...
					<li class="active">Terjual Cash</li>
				</ol>
			</div>
		</div>
							...
							<tbody>
								<?PHP
									$query = $this->db->query("SELECT idmobil,_jenismobil.data as merk,tipemobil,nopol,warna,status,tglspk,penjualan.pembeli as pembeli
											...
											WHERE mobil._jenismobil=_jenismobil.code AND mobil._deleted=0 AND status='Terjual-Cash' AND 
								...
								<tr>
									...
									<td align="center">
										...
										<a href="<?PHP echo base_url() ?>terjual_cash/detail/<?PHP echo $row->idmobil; ?>" class="fa fa-search" style="text-decoration:none;color:blue"></a> &nbsp;
									</td>
								</tr>
								<?PHP
										}
						...
						<table class="table table-striped mb30" id="table-responsive">
							...
							<tbody>
								<?PHP
									$query = $this->db->query("SELECT idmobil,_jenismobil.data as merk,tipemobil,nopol,warna,status,tglspk,penjualan.pembeli as pembeli
											FROM mobil,_jenismobil,penjualan
											WHERE mobil._jenismobil=_jenismobil.code AND mobil._deleted=0 AND status='Terjual-Cash' AND mobil.idpenjualan=penjualan.idpenjualan ORDER BY merk ASC");
									if($query->num_rows())
									{
										foreach($query->result() as $row)
										{	
